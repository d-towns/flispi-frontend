...
const SelectDropdown = ({options, onValueChange, value}: SelectDropdownProps) => {
    return (
    <Select.Root  value={value} onValueChange={(value:string) => onValueChange(value)}>
      <Select.Trigger
        ...
        aria-label="Food"
      >
        <Select.Value placeholder="Minimum Bedrooms..." />
    ...
    return (
      <Select.Item
        className={classNames(
          'text-[13px] leading-none text-violet11 rounded-[3px] flex items-center hover:bg-[#003366] hover:text-white transition ease-in-out duration-100 h-[25px] pr-[35px] pl-[25px] relative select-none data-[disabled]:text-mauve8 data-[disabled]:pointer-events-none data-[highlighted]:outline-none data-[highlighted]:bg-violet9 data-[highlighted]:text-violet1',
          className ?? ''
        )}
        value={value}
        ref={forwardedRef}
